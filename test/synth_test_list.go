...
func synthACLTestsList() []mainTestCase {
	return []mainTestCase{
		...
		},
		{
			...
			command:  "../bin/vpcgen synth acl -c %s/acl_tg_multiple_tf/config_object.json -s %s/acl_tg_multiple_tf/conn_spec.json -o %s/acl_tg_multiple_tf/nacl_expected.tf",
		},
		{
			testName: "acl_tg_multiple_tf_separate",
			command:  "../bin/vpcgen synth acl -c %s/acl_tg_multiple_tf_separate/config_object.json -s %s/acl_tg_multiple_tf_separate/conn_spec.json -d %s/acl_tg_multiple_tf_separate -f tf",
		},
	}
}
...
func synthSGTestsList() []mainTestCase {
	return []mainTestCase{
		...
		// sg tg multiple (tf separate)
		{
			...
			command:  "../bin/vpcgen synth sg -c %s/sg_tg_multiple_tf_separate/config_object.json -s %s/sg_tg_multiple_tf_separate/conn_spec.json -d %s/sg_tg_multiple_tf_separate -f tf",
		},
	}
}
