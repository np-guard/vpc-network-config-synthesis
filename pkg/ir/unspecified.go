...
func (s *Spec) computeBlockedNIFs() []string {
	...
	for instance, instanceDetails := range s.Defs.Instances {
		if s.findResourceInConnections([]string{string(instance)}, ResourceTypeNIF) {
			continue
		}
		...
		if len(blockedNIFs) > 0 && len(instanceDetails.Nifs) == 1 {
			blockedResources = append(blockedResources, string(instance))
		} else {
